# The following rmd file generates all the plots and tables corresponding to snRNA-seq analysis in Muralidharan et al 2025, "Human brain cell-type-specific aging clocks based on single-nuclei transcriptomics" (doi: https://doi.org/10.1101/2025.02.28.640749)
...
cell_type_markers_extra <- c("MOG", "MAG", "C3", "CSF1R", "GJA1", "GFAP", "COL9A1", "VCAN", "SLC17A7", "NRGN", "GAD2", "SYNPR", "VIM", "FLT1", "COL6A1", "DCN", "CD3E", "CD96", "CD247")
...
clusterProfiler::dotplot(go_ora_list$`Old vs Young`$Oligodendrocytes) +
  theme(panel.grid = element_blank()) +
  scale_color_gradient(low = "firebrick1", high = "gray94")
...
clusterProfiler::dotplot(go_ora_list$`Old vs Young`$`Excitatory Neurons`) +
  theme(panel.grid = element_blank()) +
  scale_color_gradient(low = "firebrick1", high = "gray94")
...
# Tables
## Supp Data 1b - Number of cells per sample at each stage of QC
```{r}
cell_count_table <- training_data_so_qc@meta.data %>% 
  select(sample_id) %>% 
  summarise("# of Nuclei Recovered" = n(), .by = c("sample_id"))

cell_count_table <- training_data_so_doublets@meta.data %>% 
  select(sample_id) %>% 
  summarise("# of Nuclei After Quality Control (After selecting nuclei with reads 1200 - 100000, gene counts 800 - 12000 and less than 5% mitochondrial reads)" = n(), .by = c("sample_id")) %>% 
  inner_join(cell_count_table, by = "sample_id")

cell_count_table <- training_data_so_final@meta.data %>% 
  select(sample_id) %>% 
  summarise("# of Nuclei After Doublet Removal" = n(), .by = c("sample_id")) %>% 
  inner_join(cell_count_table, by = "sample_id") %>% 
  arrange(sample_id)

cell_count_table %>% 
  data.table::fwrite(file = paste(dir_list$docs, "Supplementary_Data_1b.csv", sep = "/"))
```
...
training_data_so_final@meta.data %>% 
  ...
  select(sample_id, cell_type_levels) %>% 
  data.table::fwrite(file = paste(dir_list$docs, "Supplementary_Data_1c.csv", sep = "/"))
```
